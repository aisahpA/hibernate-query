...
/**
 * 分页对象
 * <br/>先设置页码pageNumber和每页数量pageSize，然后查询后携带查询结果results和总记录数totalCount
 * <br/>当设置startIndex的时候，会优先使用设置的值，否则将由pageNumber和pageSize计算出起点
 * 
 * @author chenxianguan  2016年1月15日
 *
 * @param <T> 主要用于泛型指定返回的数据类型
 */
public class Page<T> {

	private static final int DEFAULT_PAGE_SIZE = 10;   //每页显示默认记录数
	private static final int NON_USE_START_INDEX = -99;//不使用startIndex参数的标示值

	private int pageNumber = 1;                        // 当前页码（从1开始）
	private int pageSize = DEFAULT_PAGE_SIZE;          // 每页数据总条数
	private int startIndex = NON_USE_START_INDEX;      // 真正的查询起点
	private int totalCount = 0;                        // 总记录数
	private Collection<T> results = new ArrayList<>(); // 查询结果集
	...
	/**
	 * 设置每页的最大数量(参数为空或<=0时，设置为默认的值 {@link #DEFAULT_PAGE_SIZE})
	 * 
	...
	public List<T> getList() {		
		return results instanceof List ? 
			(List<T>) results : new ArrayList<>(results);
	}
